...
if (!agentInstall.includes("usr/lib/ndl/ndl-lxc-nesting-apparmor")) {
  errors.push("ndl-agent.install must install ndl-lxc-nesting-apparmor");
}
const lxcConfigGo = (existsSync("internal/lxc/config.go") ? readFileSync("internal/lxc/config.go", "utf8") : "") +
  (existsSync("internal/lxc/types.go") ? readFileSync("internal/lxc/types.go", "utf8") : "");
...
if (!lxcConfigGo.includes("lxc.seccomp.allow_nesting = 1") || !lxcConfigGo.includes("ndl-lxc-nesting-apparmor") || !lxcConfigGo.includes("/dev/fuse")) {
  errors.push("system containers must persist the nested-engine LXC feature set (seccomp nesting, fuse, start-host hook)");
}
if (/\nmount,/.test(lxcConfigGo) || lxcConfigGo.includes("lxc.apparmor.profile = unconfined\nlxc.apparmor.allow_nesting")) {
  errors.push("nesting must not unconfine or add a No-DAL blanket mount rule");
}
...
if (!dockerDocs.includes("lxc.seccomp.allow_nesting = 1") || !dockerDocs.includes("ndl-lxc-nesting-apparmor") || !dockerDocs.includes("nesting: true")) {
  errors.push("docs/docker.md must document the persisted nested-engine feature set");
}
...
if (!rules.includes("./cmd/ndl-ct-prepare")) {
  errors.push("debian/rules must build ndl-ct-prepare");
}
if (!rules.includes("./cmd/ndl-lxc-nesting-apparmor")) {
  errors.push("debian/rules must build ndl-lxc-nesting-apparmor");
}
...
if (!/^ExecStop=-\/usr\/bin\/lxc-stop /m.test(ctUnit)) {
  errors.push("nodal-ct@.service must ignore lxc-stop failure when the container is already stopped");
}
...
if (!/^ExecStop=-\/usr\/bin\/ctr /m.test(ociUnit)) {
  errors.push("nodal-oci@.service must ignore ctr kill when the task is already gone");
}
...
const nestingGo = existsSync("internal/lxc/nesting.go")
  ? readFileSync("internal/lxc/nesting.go", "utf8")
  : "";
if (!nestingGo.includes("PatchGeneratedNestingProfile") || !nestingGo.includes("generated by: lxc-generate-aa-rules.py")) {
  errors.push("nesting AppArmor patch must apply current LXC generated-profile nesting semantics");
}
if (!existsSync("cmd/ndl-lxc-nesting-apparmor/main.go")) {
  errors.push("missing cmd/ndl-lxc-nesting-apparmor/main.go");
}
const appliedGo = existsSync("internal/lxc/applied.go")
  ? readFileSync("internal/lxc/applied.go", "utf8")
  : "";
if (!appliedGo.includes("writeApplied") || !appliedGo.includes("Nesting")) {
  errors.push("RewriteRuntimeConfig must persist nesting on last-applied");
}
