...
- LOW. Adding other-execute on storage/workload parents still has a temporary root chown fallback. CAP_FOWNER is now in the agent bounding set for Local Host dest chmod/lchown/fchownat. Why not blocking: the fallback remains for older units; ndl-control stays CAP_NET_BIND_SERVICE only.
...
Physical harness run `ce10-phys-9` is integration evidence on this
Debian 13 homelab with a disposable Debian 13 KVM worker as node B.
It is not a complete CE 1.0 physical certification: gate 12 cannot
PASS a virtual Node B, gate 20 cannot PASS without an armed Node A
reboot, and gate 21 cannot PASS a same-version reinstall. Remaining
HIGH items below include those honesty gaps plus Cloud-engine work.
...
- HIGH. Dest agent Execute is wired for a Ready worker with a valid
  ...
  use dest CreateCT/PrepareVM/Backup when dest is Ready. Dest-listen
  offline migrate, dest object pull, dest boot, and dest guest
  reboot/autostart passed on physical node A plus disposable Debian 13
  KVM node B (run `ce10-phys-9`) as integration evidence. Two physical
  chassis were not available; gate 12 is BLOCKED-PHYSICAL until Node B
  is bare metal.
  Why remaining: a second physical chassis is a required CE 1.0
  two-node gate, not dest-agent-nil.
...
- HIGH. Pairing reuse. After a pairing token is consumed, reconnect
  ...
  Why remaining: join-token consume on two chassis is extra topology
  proof; dest-listen join on the KVM worker passed in `ce10-phys-9`.
...
executable physical-certification harness. Run `ce10-phys-9` closed
many appliance gates on this Debian 13 host plus a disposable Debian
13 KVM worker, but the hardware milestone is not closed: the second
physical node, Node A reboot/autostart, and real package-upgrade gates
remain BLOCKED.
...
- `lib.sh` enforces the absolute production-safety rules: it refuses any
  ...
  resources.
- `honesty.sh` makes a virtual Node B, an unarmed Node A reboot, and a
  same-version reinstall BLOCKED rather than PASS.
- `selftest.sh` validates the guards, ledger, verdict logic, and those
  honesty conditions in any environment and is safe for CI. On a
  non-appliance host the full harness correctly reports every hardware
  gate as BLOCKED-PHYSICAL and never a PASS.

`ce10-phys-9` remains useful integration evidence. Disposable MinIO
stood in for R2. Node B was a Debian 13 KVM guest, not a second
chassis, so gate 12 is BLOCKED-PHYSICAL. Gate 20 is BLOCKED-PHYSICAL
until an armed Node A reboot. Gate 21 is
BLOCKED-PHYSICAL/RELEASE-ARTIFACT until a real newer supported
`nodal_*.deb` exists. The installer ISO remains unbooted.
