...
Optional feature packages (`nodal-feature-oci`, `nodal-feature-docker`,
`nodal-feature-gpu`, `nodal-feature-k8s`, `nodal-feature-distributed-storage`,
`nodal-feature-ai`) are not installed by `apt-get install nodal`. Enable
them later from Add Features or `nodalctl feature enable oci`.
