...
Run `ce10-phys-9` on this Debian 13 host is **not** a complete CE 1.0
physical certification. Node B was a disposable Debian 13 KVM guest
(not a second chassis). Gate 20 rebooted that guest. Gate 21 was a
same-version `1.0.6 -> 1.0.6` reinstall. After the honesty correction,
those three gates must be `BLOCKED-PHYSICAL` or
`BLOCKED-PHYSICAL/RELEASE-ARTIFACT`. Virtual two-node dest-listen /
migrate / dest-pull work from that run remains valid integration
evidence. Disposable MinIO stood in for R2 when `CERT_R2_*` was unset.
Production guests were not mutated. Report:
...
- `packaging/cert/honesty.sh` refuses a virtual Node B, an unarmed Node A
  reboot, or a same-version reinstall as a required physical PASS.
- `packaging/cert/selftest.sh` validates the harness itself in any environment
  ...
  on `FAIL` or `BLOCKED-*`; kvm/same-version/un-rebooted cannot PASS).
  Run it in CI: `bash packaging/cert/selftest.sh`.
...
- `2` no defects, but `BLOCKED-PHYSICAL` or
  `BLOCKED-PHYSICAL/RELEASE-ARTIFACT` gates remain; not yet certified.
...
| `CERT_REBOOT_NODE_A=I-UNDERSTAND-REBOOT-PRIMARY` | arm the Node A reboot gate |
| `CERT_REBOOT_NODE_A_EXECUTE=now` | actually reboot the primary physical host |
...
Gate 12 PASSes only when Node B is proven bare metal (`systemd-detect-virt`
is `none` and DMI/sysfs do not show a guest). A KVM/QEMU/VM/container is
`BLOCKED-PHYSICAL`; the join may still be recorded as integration evidence.
...
(`dpkg --compare-versions after gt before`) that preserves users/auth,
configuration, workloads, storage metadata, cluster state, and backup
target ids. `1.0.6 -> 1.0.6` is `BLOCKED-PHYSICAL/RELEASE-ARTIFACT`.
Same-version reinstall can be recorded as integration evidence when
`CERT_REINSTALL_IDEMPOTENCE=1`.
