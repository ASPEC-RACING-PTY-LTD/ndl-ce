...
func TestCollectGPUFromPCI(t *testing.T) {
	...
	if intel.Hint != "" || nvidia.Hint != "" || amd.Hint != "" {
		t.Fatalf("must not invent DRI locators without by-path nodes: %+v", inv.GPUs)
	}
}

func TestCollectGPUDeviceHintsFromDRI(t *testing.T) {
	root := writeTree(t, map[string]string{
		"sys/bus/pci/devices/0000:13:00.0/vendor": "0x1002\n",
		"sys/bus/pci/devices/0000:13:00.0/device": "0x13c0\n",
		"sys/bus/pci/devices/0000:13:00.0/class":  "0x030000\n",
		"sys/bus/pci/devices/0000:13:00.0/uevent": "DRIVER=amdgpu\nPCI_SLOT_NAME=0000:13:00.0\n",
		"dev/dri/by-path/pci-0000:13:00.0-render": "",
		"dev/dri/by-path/pci-0000:13:00.0-card":   "",
	})
	inv := collectFixture(t, root, missingTools)
	if len(inv.GPUs) != 1 {
		t.Fatalf("gpus=%d %+v", len(inv.GPUs), inv.GPUs)
	}
	hint := inv.GPUs[0].Hint
	if !strings.Contains(hint, "/dev/dri/by-path/pci-0000:13:00.0-render") {
		t.Fatalf("hint=%q", hint)
	}
	if strings.Contains(hint, "renderD128") {
		t.Fatalf("must use by-path locators, got %q", hint)
	}
}
