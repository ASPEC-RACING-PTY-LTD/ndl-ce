...
# Native attach/xterm uses xterm-256color so Bash can enable bracketed paste.
# Getty/console keeps TERM=linux from its unit environment.
...
if [ -z "${INPUTRC:-}" ] && [ -r /etc/ndl/inputrc ]; then
  export INPUTRC=/etc/ndl/inputrc
fi
if [ -n "${BASH_VERSION:-}" ]; then
  bind 'set enable-bracketed-paste on' 2>/dev/null || true
fi
`
	if err := os.WriteFile(filepath.Join(dir, "00-ndl-term.sh"), []byte(term), 0o644); err != nil {
		return err
	}
	return ensureGuestReadline(rootfs)
}

func ensureGuestReadline(rootfs string) error {
	if strings.TrimSpace(rootfs) == "" {
		return nil
	}
	if err := os.MkdirAll(filepath.Join(rootfs, guestNDLDir), 0o755); err != nil {
	...
	body := `# No-DAL readline defaults. Included from /etc/ndl/inputrc.
# Enables Bash bracketed paste so multiline pastes keep comments and quotes.
$include /etc/inputrc
set enable-bracketed-paste on
`
	return os.WriteFile(filepath.Join(rootfs, "etc", "ndl", "inputrc"), []byte(body), 0o644)
}
...
func writeAlpineResolvFallback(rootfs string, spec Spec) error {
	if strings.TrimSpace(rootfs) == "" {
		return nil
	}
	...
	return chownMappedGuest(rootfs, spec, "etc/resolv.conf")
}
