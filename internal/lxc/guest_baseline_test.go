...
func TestGuestBaselineFiles(t *testing.T) {
	...
	term, err := os.ReadFile(filepath.Join(root, "etc", "profile.d", "00-ndl-term.sh"))
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(string(term), "enable-bracketed-paste on") || strings.Contains(string(term), "enable-bracketed-paste off") {
		t.Fatalf("guest bash must enable bracketed paste: %s", term)
	}
	inputrc, err := os.ReadFile(filepath.Join(root, "etc", "ndl", "inputrc"))
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(string(inputrc), "enable-bracketed-paste on") || !strings.Contains(string(inputrc), "$include /etc/inputrc") {
		t.Fatalf("ndl inputrc: %s", inputrc)
	}
}
