...
func TestDeleteResetsFailedUnit(t *testing.T) {
	...
	want := []string{
		BinSystemctl + " stop nodal-ct@" + id + ".service",
		BinSystemctl + " disable nodal-ct@" + id + ".service",
		BinSystemctl + " reset-failed nodal-ct@" + id + ".service",
	}
	if strings.Join(got, "\n") != strings.Join(want, "\n") {
...
func TestStartWaitsForInitPID(t *testing.T) {
	...
	if infoCalls < 5 {
		t.Fatalf("must poll lxc-info until pid and IPv4 are set, got %d", infoCalls)
	}
}
...
func TestStartFailsIfIPv4NeverAppears(t *testing.T) {
	...
	err := e.Start(context.Background(), id)
	if err == nil {
		t.Fatal("Start must fail when eth0 has no IPv4")
	}
