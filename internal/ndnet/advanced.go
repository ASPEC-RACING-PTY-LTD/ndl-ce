...
func (e *Engine) deleteNetworkAdvanced(ctx context.Context, op AdvancedOp) (AdvancedResult, error) {
	id := strings.TrimSpace(op.NetworkID)
	if id == "" {
		id = strings.TrimSpace(op.ObjectID)
	}
	if id == "" {
		return AdvancedResult{}, fmt.Errorf("network_id is required")
	}
	spec := Spec{NetworkID: id, Name: op.Name}
	if err := e.Delete(ctx, spec); err != nil {
		return AdvancedResult{}, err
	}
	return AdvancedResult{Action: ActionNetworkDelete, ObjectID: id, Status: StatusUnavailable, Reason: "deleted"}, nil
}
