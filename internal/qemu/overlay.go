...
func (e *Engine) overlayCreate(ctx context.Context, req OverlayRequest, max int) (OverlayResult, error) {
	...
	if running {
		if err := e.ensureQEMUCanCreate(req.OverlayPath); err != nil {
			return OverlayResult{}, err
		}
...
func qemuUserIDs() (int, int, error) {
	u, err := user.Lookup(QEMUUser)
	if err != nil {
		return 0, 0, fmt.Errorf("qemu user %s is missing", QEMUUser)
	}
...
// ensureQEMUCanCreate makes the overlay parent writable by ndl-qemu so live
// QMP blockdev-snapshot-sync can create the new qcow2. Directory pools stay
// root-owned; other users still cannot list or write.
func (e *Engine) ensureQEMUCanCreate(path string) error {
	dir := filepath.Dir(path)
	if dir == "." || dir == "/" {
		return fmt.Errorf("overlay parent is invalid")
	}
...
func (e *Engine) qmpExternalSnapshot(id, overlay string) error {
	...
	defer q.Close()
	_, err = q.exec("blockdev-snapshot-sync", qmpSnapshotSyncArgs(overlay))
	return err
}

func qmpSnapshotSyncArgs(overlay string) map[string]any {
	return map[string]any{
		"node-name":          "disk0",
		"snapshot-file":      overlay,
		"snapshot-node-name": "disk0-overlay",
		"format":             "qcow2",
		"mode":               "absolute-paths",
	}
}
