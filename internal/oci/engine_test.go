...
func TestEngineDeleteResetsFailedUnit(t *testing.T) {
	id := uuid.NewString()
	var got []string
	e := &Engine{
		DataDir:      t.TempDir(),
	...
	if !strings.Contains(joined, BinSystemctl+" disable "+unitName(id)) {
		t.Fatalf("missing disable: %s", joined)
	}
	if !strings.Contains(joined, BinSystemctl+" reset-failed "+unitName(id)) {
		t.Fatalf("missing reset-failed: %s", joined)
	}
}
