...
func TestTermArgvSystemContainerUsesTypedLXCAttach(t *testing.T) {
	...
	if !strings.Contains(joined, "TERM=xterm-256color") || strings.Contains(joined, "TERM=linux") {
		t.Fatal("native attach must advertise xterm so Bash enables bracketed paste")
	}
