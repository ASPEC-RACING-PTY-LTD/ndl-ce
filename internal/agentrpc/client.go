...
// DeleteNetwork tears down an isolated or isolated-nat network on the agent.
func (c Client) DeleteNetwork(ctx context.Context, spec ndnet.Spec) error {
	_, err := c.NetAdvanced(ctx, ndnet.AdvancedOp{
		Action: ndnet.ActionNetworkDelete, ObjectID: spec.NetworkID, NetworkID: spec.NetworkID, Name: spec.Name,
	})
	return err
}
...
func (c Client) CreateCT(ctx context.Context, spec lxc.Spec) (lxc.Result, error) {
	res, err := c.rpc().Execute(ctx, connect.NewRequest(&agentv1.ExecuteRequest{
		Method: &agentv1.ExecuteRequest_CtCreate{CtCreate: &agentv1.CTCreate{
			...
			SkipImage: spec.SkipImage, NoStart: spec.NoStart, IpConfigJson: mustIPJSON(spec.IP),
		}},
	}))
	if err != nil {
...
func (c Client) LifecycleCT(ctx context.Context, req lxc.LifecycleRequest) (lxc.Result, error) {
	msg := &agentv1.CTLifecycle{
		...
		IpConfigJson: mustIPJSON(req.IP), Mac: req.MAC, Extras: append([]string{}, req.Extras...),
	}
