...
func TestCTUnitPreparesDirectoryRootMount(t *testing.T) {
	...
	if !strings.Contains(text, "ExecStop=-/usr/bin/lxc-stop -P /var/lib/ndl/runtime/lxc -n %i --timeout 30") {
		t.Fatal("nodal-ct@.service must ignore lxc-stop when the container is already stopped")
	}
	if regexp.MustCompile(`(?m)^ExecStop=/usr/bin/lxc-stop`).MatchString(text) {
		t.Fatal("ExecStop without '-' marks a dead container as a failed unit")
	}
