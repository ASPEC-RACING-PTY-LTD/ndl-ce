...
func TestApplyDesiredSessionDialsAfterJoinMaterial(t *testing.T) {
	var got map[string]any
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/v1/cluster/sessions" {
			http.NotFound(w, r)
	...
	ident := identity.Files{Dir: root}
	if err := ident.SaveJoinMaterial("c1", "n1", caPEM, certPEM, keyPEM); err != nil {
		t.Fatal(err)
	}
	eng := &ndnet.Engine{
		Root: root, NetworkDir: filepath.Join(root, "etc/systemd/network"),
	...
	if got["peer_id"] != id {
		t.Fatalf("post-join OpenSession dial failed: %v", got)
	}
}
