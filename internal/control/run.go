...
func Run(cfg Config) error {
	...
	if err := store.Apply(bootCtx, store.SQLDB{DB: db}, files); err != nil {
		bootCancel()
		return err
	}
	st := &appdb.Postgres{DB: db}
	if err := ensureReady(bootCtx, st, cfg.SetupHash); err != nil {
		bootCancel()
		return err
	}
	bootCancel()
	...
	if err := lease.acquire(leaseCtx); err != nil {
		leaseCancel()
	...
	recCtx, recCancel := context.WithTimeout(context.Background(), 20*time.Second)
	if n := srv.ReconcileInterruptedBackupRuns(recCtx); n > 0 {
...
func readCertificateEnabled(st appdb.Store) (bool, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	return certificateEnabled(ctx, st)
}
