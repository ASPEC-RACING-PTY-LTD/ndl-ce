...
func TestEnsureFailsWhenParentIsAFile(t *testing.T) {
	dir := t.TempDir()
	parent := filepath.Join(dir, "not-a-dir")
	if err := os.WriteFile(parent, []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	ca := CA{Dir: filepath.Join(parent, "cluster-ca")}
	if err := ca.Ensure(time.Now().UTC()); err == nil {
		t.Fatal("Ensure must fail when it cannot create the CA directory")
	}
}
