...
func TestUpdateArgvNeverShell(t *testing.T) {
	...
	ociFeat, err := FeatureInstallArgv("nodal-feature-oci", false)
	if err != nil || contains(ociFeat, "containerd") || contains(ociFeat, "systemctl") {
		t.Fatalf("oci feature argv must name only the metapackage: %v %v", ociFeat, err)
	}
