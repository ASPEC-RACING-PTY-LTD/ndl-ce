...
UPDATE workloads SET cpus=COALESCE(NULLIF($2,0), cpus),
  ...
  updated_at=now()
WHERE id=$1`, w.ID, w.CPUs, w.MemoryBytes, w.DesiredPower)
	return err
}
...
func (p *Postgres) CreateWorkloadDisk(ctx context.Context, d WorkloadDisk) error {
	...
	_, err := p.DB.ExecContext(ctx, `
INSERT INTO workload_disks (id, cluster_id, workload_id, volume_id, role, created_at)
VALUES ($1,$2,$3,$4,$5,$6)`, d.ID, d.ClusterID, d.WorkloadID, d.VolumeID, d.Role, d.CreatedAt)
	return err
}

func (p *Postgres) ListWorkloadDisks(ctx context.Context, clusterID, workloadID string) ([]WorkloadDisk, error) {
	rows, err := p.DB.QueryContext(ctx, `
SELECT id::text, cluster_id::text, workload_id::text, volume_id::text, role, created_at
...
ORDER BY created_at`, clusterID, workloadID)
	...
	for rows.Next() {
		var d WorkloadDisk
		if err := rows.Scan(&d.ID, &d.ClusterID, &d.WorkloadID, &d.VolumeID, &d.Role, &d.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, d)
	}
...
func (p *Postgres) CreateWorkloadNIC(ctx context.Context, n WorkloadNIC) error {
	...
	_, err := p.DB.ExecContext(ctx, `
INSERT INTO workload_nics (id, cluster_id, workload_id, network_id, mac, ipv4, created_at)
VALUES ($1,$2,$3,$4,$5,$6,$7)`, n.ID, n.ClusterID, n.WorkloadID, n.NetworkID, n.MAC, n.IPv4, n.CreatedAt)
	return err
}

func (p *Postgres) ListWorkloadNICs(ctx context.Context, clusterID, workloadID string) ([]WorkloadNIC, error) {
	rows, err := p.DB.QueryContext(ctx, `
SELECT id::text, cluster_id::text, workload_id::text, network_id::text, mac, ipv4, created_at
...
ORDER BY created_at`, clusterID, workloadID)
	...
	for rows.Next() {
		var n WorkloadNIC
		if err := rows.Scan(&n.ID, &n.ClusterID, &n.WorkloadID, &n.NetworkID, &n.MAC, &n.IPv4, &n.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, n)
	}
...
func (p *Postgres) UpdateWorkloadNIC(ctx context.Context, n WorkloadNIC) error {
	_, err := p.DB.ExecContext(ctx, `UPDATE workload_nics SET ipv4=$2 WHERE id=$1`, n.ID, n.IPv4)
	return err
}
...
func scanWorkload(row rowScanner) (Workload, error) {
	...
	var blockers, devices, warn []byte
