...
func (p *Postgres) DeleteVolume(ctx context.Context, clusterID, volumeID string) error {
	...
	if _, err := tx.ExecContext(ctx, `DELETE FROM snapshots WHERE volume_id=$1`, volumeID); err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM workload_disks WHERE cluster_id=$1 AND volume_id=$2`, clusterID, volumeID); err != nil {
		return err
	}
	res, err := tx.ExecContext(ctx, `DELETE FROM volumes WHERE cluster_id=$1 AND id=$2`, clusterID, volumeID)
	if err != nil {
		return err
	}
