...
func (p *Postgres) DeleteNetwork(ctx context.Context, clusterID, id string) error {
	tx, err := p.DB.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	defer func() { _ = tx.Rollback() }()
	if _, err := tx.ExecContext(ctx, `DELETE FROM addresses WHERE cluster_id=$1 AND network_id=$2`, clusterID, id); err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM dhcp_reservations WHERE cluster_id=$1 AND network_id=$2`, clusterID, id); err != nil {
		return err
	}
	res, err := tx.ExecContext(ctx, `DELETE FROM networks WHERE cluster_id=$1 AND id=$2`, clusterID, id)
	if err != nil {
		return err
	}
