...
func (m *Memory) DeleteNetwork(_ context.Context, clusterID, id string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	n, ok := m.networks[id]
	if !ok || n.ClusterID != clusterID {
	...
	for _, nic := range m.workloadNICs {
		if nic.NetworkID == id {
			return fmt.Errorf("network is still attached to a workload")
		}
	}
	for addrID, a := range m.addresses {
		if a.NetworkID == id {
			delete(m.addresses, addrID)
		}
	}
	for resID, r := range m.reservations {
		if r.NetworkID == id {
			delete(m.reservations, resID)
		}
	}
	for vlanID, v := range m.netVLANs {
		if v.NetworkID == id {
			delete(m.netVLANs, vlanID)
		}
	}
