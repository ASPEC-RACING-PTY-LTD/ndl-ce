...
func (s *Server) lifecycleWorkload(action string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		rpc, ok := s.workloadsOn(r.Context(), p.User.ClusterID, s.ownerNodeID(*row))
		if !ok || rpc == nil {
			writeErr(w, http.StatusBadGateway, "workload agent is unavailable")
			return
		}
		...
		res, err := rpc.LifecycleCT(r.Context(), req)
		if err != nil {
			s.finishOp(r.Context(), op, "failed", err.Error(), 0)
			writeErr(w, http.StatusBadRequest, err.Error())
			return
...
func (s *Server) deleteSystemContainer(ctx context.Context, p *principal, row appdb.Workload) error {
	rpc, ok := s.workloadsOn(ctx, p.User.ClusterID, s.ownerNodeID(row))
	if !ok || rpc == nil {
	...
	if _, err := rpc.LifecycleCT(ctx, lxc.LifecycleRequest{WorkloadID: row.ID, Action: "delete"}); err != nil {
...
func (s *Server) patchWorkload(w http.ResponseWriter, r *http.Request) {
	...
	if row.Kind == oci.KindOCI {
		...
		if powerChanged && next.DesiredPower == "stopped" {
			if _, err := rpc.LifecycleCT(r.Context(), lxc.LifecycleRequest{WorkloadID: row.ID, Action: "stop"}); err != nil {
				writeErr(w, statusFor(err), err.Error())
				return
			}
			...
			if _, err := rpc.LifecycleCT(r.Context(), lxc.LifecycleRequest{WorkloadID: row.ID, Action: "start"}); err != nil {
				writeErr(w, statusFor(err), err.Error())
				return
			}
		} else if req.RestartAfterSave && running {
			if _, err := rpc.LifecycleCT(r.Context(), lxc.LifecycleRequest{WorkloadID: row.ID, Action: "restart"}); err != nil {
				writeErr(w, statusFor(err), err.Error())
				return
			}
			next.PendingRestart = false
...
func (s *Server) refreshWorkloads(ctx context.Context, clusterID string) {
	...
	if s.Workloads != nil && len(local) > 0 {
		obs, oerr := s.Workloads.GetWorkloads(ctx, appdb.WorkloadHints(local))
		if oerr == nil {
			_, _, _ = appdb.ReconcileWorkloads(ctx, s.Store, clusterID, local, obs)
		}
	}
	for nid, wls := range byDest {
		rpc, ok := s.workloadsOn(ctx, clusterID, nid)
		if !ok || rpc == nil {
			continue
		}
		dctx, cancel := context.WithTimeout(ctx, 3*time.Second)
		obs, oerr := rpc.GetWorkloads(dctx, appdb.WorkloadHints(wls))
		cancel()
		if oerr != nil {
			continue
		}
		_, _, _ = appdb.ReconcileWorkloads(ctx, s.Store, clusterID, wls, obs)
	}
}
