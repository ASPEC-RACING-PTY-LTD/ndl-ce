...
func (s *Server) ociLifecycle(w http.ResponseWriter, r *http.Request, p *principal, row appdb.Workload, action string) {
	...
	if action == "delete" {
		if !strings.EqualFold(row.Status, oci.StatusUnavailable) {
			if _, err := rpc.LifecycleOCI(r.Context(), oci.LifecycleRequest{WorkloadID: row.ID, Action: action}); err != nil {
		...
		if err := s.Store.DeleteWorkload(r.Context(), p.User.ClusterID, row.ID); err != nil {
			s.finishOp(r.Context(), op, "failed", err.Error(), 0)
			writeErr(w, http.StatusInternalServerError, "could not record container delete")
			return
		}
		gone, err := s.Store.GetWorkload(r.Context(), p.User.ClusterID, row.ID)
		if err != nil || gone != nil {
			s.finishOp(r.Context(), op, "failed", "could not record container delete", 0)
			writeErr(w, http.StatusInternalServerError, "could not record container delete")
			return
		}
