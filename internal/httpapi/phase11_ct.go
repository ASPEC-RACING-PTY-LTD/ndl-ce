...
func (s *Server) restoreSystemContainer(ctx context.Context, clusterID string, src *appdb.Workload, art appdb.BackupArtifact, mode string, dest *appdb.Node) (string, error) {
	art = s.bootableCTArtifact(ctx, clusterID, art)
...
func (s *Server) restoreNewCT(ctx context.Context, clusterID string, src *appdb.Workload, art appdb.BackupArtifact, dest *appdb.Node) (string, error) {
	...
	if netID == "" {
		var nerr error
		netID, bridge, nerr = s.pickRestoreNetwork(ctx, clusterID, src)
		if nerr != nil {
			return "", nerr
		}
	} else if netw, err := s.Store.GetNetwork(ctx, clusterID, netID); err == nil && netw != nil && isProductionLAN(netw) {
		var nerr error
		netID, bridge, nerr = s.pickRestoreNetwork(ctx, clusterID, src)
		if nerr != nil {
			return "", nerr
	...
	if apply {
		...
		rootfs = loc
		if !local {
			if strings.HasPrefix(strings.ToLower(strings.TrimSpace(art.PullURL)), "http") {
				if destBK, ok := s.destBackup(ctx, dest); ok {
					if _, err := destBK.CopyBackup(ctx, qemu.BackupMkdir, "", rootfs); err != nil {
						return "", err
					}
				}
				if err := s.destPullVolume(ctx, dest, migrate.VolumeCopy{SourcePath: art.PullURL, DestPath: rootfs, VolumeID: newVolID}); err != nil {
					return "", err
				}
			} else if err := s.pullDestRestoredTree(ctx, dest, rootfs, srcPath); err != nil {
				return "", err
			}
...
func (s *Server) restoreNewCTV2(ctx context.Context, clusterID string, src *appdb.Workload, art appdb.BackupArtifact, dest *appdb.Node) (string, error) {
	...
	netID, bridge, err := s.pickRestoreNetwork(ctx, clusterID, src)
	if err != nil {
		return "", err
	}
	if dest != nil && apply {
		if netw, nerr := s.Store.GetNetwork(ctx, clusterID, netID); nerr == nil && netw != nil {
			if b, aerr := s.ensureDestIsolatedNet(ctx, dest, netw); aerr != nil {
				return "", aerr
			} else if b != "" {
				bridge = b
			}
	...
	if len(bp.Interfaces) > 0 && src != nil {
		if nics, _ := s.Store.ListWorkloadNICs(ctx, clusterID, src.ID); len(nics) > 0 {
			if netw, nerr := s.Store.GetNetwork(ctx, clusterID, nics[0].NetworkID); nerr == nil && netw != nil && !isProductionLAN(netw) {
				netID = netw.ID
				bridge = netw.BridgeName
				if dest != nil && apply {
					if b, aerr := s.ensureDestIsolatedNet(ctx, dest, netw); aerr != nil {
	...
	if apply {
		...
		if !local {
			// Restore V2 on the control node (local repo + target), then dest-pull
			// the tree. Dest cannot reach a control-localhost MinIO/R2 endpoint.
			if s.Backup == nil {
				return "", errUnavailable("backup agent is unavailable")
			}
			staging := filepath.Join("/var/lib/ndl/control", "tmp", "restore-"+newID)
			if err := os.MkdirAll(staging, 0o750); err != nil {
				return "", err
			}
			defer func() { _, _ = s.Backup.CopyBackup(ctx, qemu.BackupRmTree, "", staging) }()
			if err := s.restoreV2RootOn(ctx, s.Backup, art, staging, tgt); err != nil {
				return "", err
			}
			if err := s.pullDestRestoredTree(ctx, dest, rootfs, staging); err != nil {
				return "", err
			}
		} else if backupRPC != nil {
...
func isProductionLAN(n *appdb.Network) bool {
	if n == nil {
		return false
	}
	if n.Kind == "lan-bridge" || strings.EqualFold(n.Name, "lan") {
		return true
	}
	return false
}

func (s *Server) pickRestoreNetwork(ctx context.Context, clusterID string, src *appdb.Workload) (netID, bridge string, err error) {
	if src != nil {
		if nics, _ := s.Store.ListWorkloadNICs(ctx, clusterID, src.ID); len(nics) > 0 {
			if netw, nerr := s.Store.GetNetwork(ctx, clusterID, nics[0].NetworkID); nerr == nil && netw != nil && !isProductionLAN(netw) {
				return netw.ID, netw.BridgeName, nil
			}
		}
	}
	id, ierr := s.isolatedNetworkID(ctx, clusterID)
	if ierr != nil {
		return "", "", ierr
	}
	netw, nerr := s.Store.GetNetwork(ctx, clusterID, id)
	if nerr != nil || netw == nil {
		return "", "", errUnprocessable("no isolated network is available for restore")
	}
	return netw.ID, netw.BridgeName, nil
}

func (s *Server) bootableCTArtifact(ctx context.Context, clusterID string, art appdb.BackupArtifact) appdb.BackupArtifact {
	if strings.EqualFold(art.CaptureMode, appdb.BackupCaptureFull) {
		return art
	}
	items, err := s.Store.ListBackupArtifactsForWorkload(ctx, clusterID, art.WorkloadID, "")
	if err != nil {
		return art
	}
	var best *appdb.BackupArtifact
	for i := range items {
		if !strings.EqualFold(items[i].CaptureMode, appdb.BackupCaptureFull) {
			continue
		}
		if best == nil || items[i].CreatedAt.After(best.CreatedAt) {
			cp := items[i]
			best = &cp
		}
	}
