...
func TestLVMVolumeCreateFailsClosedForTinySize(t *testing.T) {
	s, mem, token := testServer(t)
	s.LVM = &fakeLVM{}
	cluster, _ := mem.GetCluster(context.Background())
	seedNode(t, mem, cluster.ID, debianInv(), false)
	...
	_ = volRes.Body.Close()
	if volRes.StatusCode != http.StatusBadRequest {
		t.Fatalf("tiny lv %d %s", volRes.StatusCode, raw)
	}
	if !strings.Contains(string(raw), storage.ErrInvalidSize.Error()) {
		t.Fatalf("tiny lv body %s", raw)
	}
	items, _ := mem.ListVolumes(context.Background(), cluster.ID, poolID)
	if len(items) != 0 {
		t.Fatalf("GET must not list a thin LV apply cannot create: %+v", items)
	}
}

func TestPhase25VMCreateUsesThinLV(t *testing.T) {
	s, mem, token := testServer(t)
	fl := &fakeLVM{}
	s.LVM = fl
	cluster, _ := mem.GetCluster(context.Background())
	...
	defer ts.Close()
	cookie := claimAdmin(t, ts, token)
	body := `{"name":"lvm-vm","kind":"vm","pool_id":"` + poolID + `","network_id":"` + netID + `"}`
	...
	created := 0
	for _, op := range fl.calls {
		if op.Action == "create-volume" {
			created++
		}
	}
	if created != 1 {
		t.Fatalf("create-volume calls %d %+v", created, fl.calls)
	}
	...
	if vol == nil || vol.BackendType != storage.BackendLVM {
		t.Fatalf("boot volume %+v", vol)
	}
	if err := storage.ValidateLVMDevice(vol.BackendRef); err != nil {
		t.Fatalf("boot BackendRef must be a thin LV: %s %v", vol.BackendRef, err)
	}
	if strings.Contains(vol.BackendRef, "volumes/vm-disk/") {
		t.Fatalf("boot must not be a directory qcow2 on the LVM pool: %s", vol.BackendRef)
	}
	if len(vm.launch.Disks) != 1 || vm.launch.Disks[0].Path != vol.BackendRef || vm.launch.Disks[0].Format != "raw" {
		t.Fatalf("launch must attach the thin LV: %+v want %s", vm.launch.Disks, vol.BackendRef)
	}
}

func TestPhase25VMCreateFailsClosedWhenLVMUnavailable(t *testing.T) {
	s, mem, token := testServer(t)
	s.LVM = &fakeLVM{status: storage.StatusUnavailable, reason: storage.LVMMissing}
	cluster, _ := mem.GetCluster(context.Background())
	node := seedNode(t, mem, cluster.ID, debianInv(), false)
	...
	poolID := uuid.NewString()
	if err := mem.CreateStoragePool(context.Background(), appdb.StoragePool{
		ID: poolID, ClusterID: cluster.ID, NodeID: node.ID, Name: "ndlvg-down",
		BackendType: storage.BackendLVM, Status: storage.StatusAvailable,
		RootPath: storage.LVMMountRoot + "/ndlvg-down",
	}); err != nil {
		t.Fatal(err)
	}
	s.Storage = fakeStorage{vol: storage.CreateVolumeResult{Handle: storage.VolumeHandle{
	...
	defer ts.Close()
	cookie := claimAdmin(t, ts, token)
	body := `{"name":"lvm-down","kind":"vm","pool_id":"` + poolID + `","network_id":"` + netID + `"}`
	req, _ := http.NewRequest("POST", ts.URL+"/api/v1/workloads", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(&http.Cookie{Name: sessionCookie, Value: cookie})
	res, err := ts.Client().Do(req)
	...
	if res.StatusCode != http.StatusUnprocessableEntity {
		t.Fatalf("unavailable LVM %d %s", res.StatusCode, raw)
	}
	if !strings.Contains(string(raw), storage.LVMMissing) {
		t.Fatalf("unavailable LVM body %s", raw)
	}
	items, _ := mem.ListWorkloads(context.Background(), cluster.ID)
	if len(items) != 0 {
		t.Fatalf("GET must not list a VM whose thin LV apply cannot create: %+v", items)
	}
}

func TestPhase25VMCloneExportAndTemplateFailClosedForLVM(t *testing.T) {
	s, mem, token := testServer(t)
	s.LVM = &fakeLVM{}
	cluster, _ := mem.GetCluster(context.Background())
	node := seedNode(t, mem, cluster.ID, debianInv(), false)
	...
	defer ts.Close()
	cookie := claimAdmin(t, ts, token)
	body := `{"name":"lvm-src","kind":"vm","pool_id":"` + poolID + `","network_id":"` + netID + `"}`
	req, _ := http.NewRequest("POST", ts.URL+"/api/v1/workloads", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(&http.Cookie{Name: sessionCookie, Value: cookie})
	res, err := ts.Client().Do(req)
	...
	clone, _ := http.NewRequest("POST", ts.URL+"/api/v1/workloads/"+id+"/clone", strings.NewReader(`{"name":"lvm-clone"}`))
	...
	if !strings.Contains(string(craw), "LVM-thin pools do not store directory qcow2 copies") {
	...
	if !strings.Contains(string(traw), lvmTemplateReason) {
	...
	if len(libs) != 0 {
		t.Fatalf("GET /images must not list an export under the LVM mount: %+v", libs)
	}
	snaps, _ := mem.ListSnapshots(context.Background(), cluster.ID, id)
	if len(snaps) != 0 {
		t.Fatalf("GET snapshots must not list a qcow2 template overlay on a thin LV: %+v", snaps)
	}
	vol, _ := mem.GetVolume(context.Background(), cluster.ID, disks[0].VolumeID)
	if vol == nil || vol.BackendRef != bootRef || storage.ValidateLVMDevice(vol.BackendRef) != nil {
		t.Fatalf("boot locator must stay a thin LV: %+v want %s", vol, bootRef)
	}
}

func TestPhase25VMRestoreFailsClosedForLVM(t *testing.T) {
	s, mem, token := testServer(t)
	s.LVM = &fakeLVM{}
	cluster, _ := mem.GetCluster(context.Background())
	node := seedNode(t, mem, cluster.ID, debianInv(), false)
	...
	defer ts.Close()
	cookie := claimAdmin(t, ts, token)
	body := `{"name":"lvm-restore","kind":"vm","pool_id":"` + poolID + `","network_id":"` + netID + `"}`
	req, _ := http.NewRequest("POST", ts.URL+"/api/v1/workloads", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(&http.Cookie{Name: sessionCookie, Value: cookie})
	res, err := ts.Client().Do(req)
	...
	if !strings.Contains(string(nraw), "LVM-thin pools do not store directory qcow2 copies") {
	...
	if !strings.Contains(string(rraw), "LVM-thin pools do not store directory qcow2 copies") {
		t.Fatalf("restore replace body %s", rraw)
	}
	for _, c := range fb.copies {
		if c[0] == "replace" {
			t.Fatalf("replace must not qemu-img onto the thin LV: %+v", fb.copies)
		}
	}
