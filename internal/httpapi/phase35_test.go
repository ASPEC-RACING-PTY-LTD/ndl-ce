...
func TestPhase35WorkloadCountCountsCTAndVM(t *testing.T) {
	s, mem, token := testServer(t)
	s.Update = &fakeUpdate{supported: true}
	cluster, _ := mem.GetCluster(context.Background())
	node := seedNode(t, mem, cluster.ID, debianInv(), false)
	for _, wl := range []appdb.Workload{
		{ID: uuid.NewString(), ClusterID: cluster.ID, NodeID: node.ID, OwnerNodeID: node.ID, DesiredNodeID: node.ID, Name: "ct-a", Kind: "system-container", Status: "running"},
		{ID: uuid.NewString(), ClusterID: cluster.ID, NodeID: node.ID, OwnerNodeID: node.ID, DesiredNodeID: node.ID, Name: "ct-b", Kind: "system-container", Status: "failed"},
		{ID: uuid.NewString(), ClusterID: cluster.ID, NodeID: node.ID, OwnerNodeID: node.ID, DesiredNodeID: node.ID, Name: "vm-a", Kind: "vm", Status: "stopped"},
		{ID: uuid.NewString(), ClusterID: cluster.ID, NodeID: node.ID, OwnerNodeID: node.ID, DesiredNodeID: node.ID, Name: "oci-a", Kind: "oci", Status: "running"},
	} {
	...
	var listed struct {
		Items []struct {
			ID            string  `json:"id"`
			WorkloadCount float64 `json:"workload_count"`
		} `json:"items"`
	}
	if err := json.Unmarshal(raw, &listed); err != nil {
		t.Fatal(err)
	...
	for _, item := range listed.Items {
		got[item.ID] = item.WorkloadCount
	}
	if got["ct"] != 2 || got["vm"] != 1 || got["oci"] != 1 {
		t.Fatalf("counts %+v from %s", got, raw)
	}
}
