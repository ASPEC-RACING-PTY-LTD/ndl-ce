...
type NetworkRPC interface {
	...
	DeleteNetwork(ctx context.Context, spec ndnet.Spec) error
...
func (s *Server) createNetwork(w http.ResponseWriter, r *http.Request) {
	...
	if ndnet.Isolated(spec.Kind) {
		want := strings.TrimSpace(spec.IPv4CIDR)
		if want == "" {
			want = ndnet.DefaultIPv4
		}
		existing, _ := s.Store.ListNetworks(r.Context(), p.User.ClusterID)
		for _, n := range existing {
			if !ndnet.Isolated(n.Kind) {
				continue
			}
			got := strings.TrimSpace(n.IPv4CIDR)
			if got == "" {
				continue
			}
			if ndnet.SameIPv4Net(want, got) {
				writeErr(w, http.StatusConflict, "isolated subnet already exists")
				return
			}
		}
	}
...
func (s *Server) deleteNetwork(w http.ResponseWriter, r *http.Request) {
	...
	if n.Kind == ndnet.KindLANBridge || strings.EqualFold(strings.TrimSpace(n.Name), "lan") {
		writeErr(w, http.StatusConflict, "LAN-bridge networks cannot be deleted")
		return
	}
	if !ndnet.Isolated(n.Kind) {
		writeErr(w, http.StatusBadRequest, "only isolated and isolated-nat networks can be deleted")
		return
	}
	nics, err := s.Store.ListWorkloadNICs(r.Context(), p.User.ClusterID, "")
	if err != nil {
		writeErr(w, http.StatusInternalServerError, err.Error())
		return
	}
	for _, nic := range nics {
		if nic.NetworkID == n.ID {
			writeErr(w, http.StatusConflict, "network is still attached to a workload")
			return
		}
	}
	if s.Network == nil {
	...
	spec := ndnet.Spec{NetworkID: n.ID, Name: n.Name, Kind: n.Kind, IPv4CIDR: n.IPv4CIDR}
	if err := s.Network.DeleteNetwork(r.Context(), spec); err != nil {
		s.audit(r, p.User.ClusterID, p.User.ID, "network.delete", "denied", err.Error())
		writeErr(w, http.StatusBadRequest, err.Error())
		return
	}
	if err := s.Store.DeleteNetwork(r.Context(), p.User.ClusterID, n.ID); err != nil {
		writeErr(w, http.StatusInternalServerError, err.Error())
		return
	}
	s.audit(r, p.User.ClusterID, p.User.ID, "network.delete", "ok", n.ID)
	s.emitEvent(r.Context(), p.User.ClusterID, n.NodeID, "network.deleted", map[string]string{"network_id": n.ID, "kind": n.Kind})
