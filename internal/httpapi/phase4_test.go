...
func (f fakeNet) DeleteNetwork(context.Context, ndnet.Spec) error {
	return f.err
}
...
func TestNetworkCreateRefusesOverlappingIsolatedCIDR(t *testing.T) {
	s, mem, token := testServer(t)
	cluster, _ := mem.GetCluster(context.Background())
	nodeID := uuid.NewString()
	_ = mem.UpsertNode(context.Background(), appdb.Node{ID: nodeID, ClusterID: cluster.ID, Name: "local"})
	_ = mem.CreateNetwork(context.Background(), appdb.Network{
		ID: uuid.NewString(), ClusterID: cluster.ID, NodeID: nodeID, Name: "already",
		Kind: ndnet.KindIsolatedNAT, Status: ndnet.StatusAvailable, IPv4CIDR: "10.77.0.0/24",
	})
	s.Network = fakeNet{
		preview: ndnet.Preview{Kind: ndnet.KindIsolatedNAT, Danger: ndnet.DangerSafe, DHCP: true},
		apply:   ndnet.ApplyResult{Kind: ndnet.KindIsolatedNAT, Status: ndnet.StatusAvailable, DHCP: true},
	}
	ts := httptest.NewServer(s.Handler())
	defer ts.Close()
	cookie := claimAdmin(t, ts, token)
	req, _ := http.NewRequest("POST", ts.URL+"/api/v1/networks", strings.NewReader(`{"name":"again","kind":"isolated-nat","ipv4_cidr":"10.77.0.0/24"}`))
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(&http.Cookie{Name: sessionCookie, Value: cookie})
	res, err := ts.Client().Do(req)
	if err != nil {
	...
	_ = res.Body.Close()
	if res.StatusCode != http.StatusConflict || !strings.Contains(string(body), "isolated subnet already exists") {
		t.Fatalf("overlap %d %s", res.StatusCode, body)
	}
}

func TestNetworkDeleteIsolatedAndRefusesLAN(t *testing.T) {
	s, mem, token := testServer(t)
	cluster, _ := mem.GetCluster(context.Background())
	nodeID := uuid.NewString()
	_ = mem.UpsertNode(context.Background(), appdb.Node{ID: nodeID, ClusterID: cluster.ID, Name: "local"})
	...
	_ = mem.CreateNetwork(context.Background(), appdb.Network{
		ID: isoID, ClusterID: cluster.ID, NodeID: nodeID, Name: "cert-drop",
		Kind: ndnet.KindIsolatedNAT, Status: ndnet.StatusAvailable, IPv4CIDR: "10.77.40.0/24",
	})
	_ = mem.CreateNetwork(context.Background(), appdb.Network{
		ID: lanID, ClusterID: cluster.ID, NodeID: nodeID, Name: "lan",
		Kind: ndnet.KindLANBridge, Status: ndnet.StatusAvailable,
	})
	s.Network = fakeNet{}
	ts := httptest.NewServer(s.Handler())
	defer ts.Close()
	...
	req, _ := http.NewRequest(http.MethodDelete, ts.URL+"/api/v1/networks/"+lanID, nil)
	req.AddCookie(&http.Cookie{Name: sessionCookie, Value: cookie})
	res, err := ts.Client().Do(req)
	if err != nil {
		t.Fatal(err)
	...
	body, _ := io.ReadAll(res.Body)
	_ = res.Body.Close()
	if res.StatusCode != http.StatusConflict || !strings.Contains(string(body), "cannot be deleted") {
		t.Fatalf("lan delete %d %s", res.StatusCode, body)
	}

	req, _ = http.NewRequest(http.MethodDelete, ts.URL+"/api/v1/networks/"+isoID, nil)
	req.AddCookie(&http.Cookie{Name: sessionCookie, Value: cookie})
	res, err = ts.Client().Do(req)
	if err != nil {
		t.Fatal(err)
	...
	if res.StatusCode != http.StatusNoContent {
		t.Fatalf("isolated delete %d", res.StatusCode)
	}
	if got, _ := mem.GetNetwork(context.Background(), cluster.ID, isoID); got != nil {
		t.Fatalf("isolated row remains: %+v", got)
	}
	if got, _ := mem.GetNetwork(context.Background(), cluster.ID, lanID); got == nil {
		t.Fatal("lan row was deleted")
	}
}

func TestNetworkDeleteRefusesAttachedNIC(t *testing.T) {
	s, mem, token := testServer(t)
	cluster, _ := mem.GetCluster(context.Background())
	nodeID := uuid.NewString()
	_ = mem.UpsertNode(context.Background(), appdb.Node{ID: nodeID, ClusterID: cluster.ID, Name: "local"})
	...
	wlID := uuid.NewString()
	_ = mem.CreateNetwork(context.Background(), appdb.Network{
		ID: netID, ClusterID: cluster.ID, NodeID: nodeID, Name: "busy",
		Kind: ndnet.KindIsolated, Status: ndnet.StatusAvailable,
	})
	_ = mem.CreateWorkload(context.Background(), appdb.Workload{
		ID: wlID, ClusterID: cluster.ID, Name: "ct", Kind: "system-container", Status: "running",
	})
	_ = mem.CreateWorkloadNIC(context.Background(), appdb.WorkloadNIC{
		ID: uuid.NewString(), ClusterID: cluster.ID, WorkloadID: wlID, NetworkID: netID, MAC: "aa:aa:aa:aa:aa:09",
	})
	s.Network = fakeNet{}
	ts := httptest.NewServer(s.Handler())
	defer ts.Close()
	cookie := claimAdmin(t, ts, token)
	req, _ := http.NewRequest(http.MethodDelete, ts.URL+"/api/v1/networks/"+netID, nil)
	req.AddCookie(&http.Cookie{Name: sessionCookie, Value: cookie})
	res, err := ts.Client().Do(req)
	if err != nil {
		t.Fatal(err)
	...
	body, _ := io.ReadAll(res.Body)
	_ = res.Body.Close()
	if res.StatusCode != http.StatusConflict || !strings.Contains(string(body), "still attached") {
		t.Fatalf("attached %d %s", res.StatusCode, body)
	}
}
