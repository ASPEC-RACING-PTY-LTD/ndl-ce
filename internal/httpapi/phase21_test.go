...
func TestPhase21OCIDeleteRemovesRow(t *testing.T) {
	_, mem, ts, cookie, clusterID, fo := phase21Ready(t)
	cluster, _ := mem.GetCluster(context.Background())
	node, _ := mem.GetNode(context.Background(), cluster.ID)
	id := uuid.NewString()
	if err := mem.CreateWorkload(context.Background(), appdb.Workload{
		ID: id, ClusterID: clusterID, NodeID: node.ID, OwnerNodeID: node.ID,
		Name: "cert-web", Kind: oci.KindOCI, Status: oci.StatusRunning,
	}); err != nil {
		t.Fatal(err)
	}
	...
	if fo.lastLife.Action != "delete" {
		t.Fatalf("OCI delete must stop the runtime: %+v", fo.lastLife)
	}
	got, _ := mem.GetWorkload(context.Background(), clusterID, id)
	if got != nil {
		t.Fatalf("OCI delete must remove the workload row: %+v", got)
	}

	goneID := uuid.NewString()
	if err := mem.CreateWorkload(context.Background(), appdb.Workload{
		ID: goneID, ClusterID: clusterID, NodeID: node.ID, OwnerNodeID: node.ID,
		Name: "cert-web-gone", Kind: oci.KindOCI, Status: oci.StatusUnavailable,
	}); err != nil {
		t.Fatal(err)
	}
	fo.lastLife = oci.LifecycleRequest{}
	del, _ = http.NewRequest("POST", ts.URL+"/api/v1/workloads/"+goneID+"/delete", strings.NewReader("{}"))
	del.Header.Set("Content-Type", "application/json")
	del.Header.Set("X-Nodal-Confirm", "delete")
	del.AddCookie(&http.Cookie{Name: sessionCookie, Value: cookie})
	res, err = ts.Client().Do(del)
	...
	_ = res.Body.Close()
	if res.StatusCode != http.StatusOK {
		t.Fatalf("unavailable OCI delete %d", res.StatusCode)
	}
	if fo.lastLife.Action == "delete" {
		t.Fatal("already-unavailable OCI delete must not require a live runtime")
	}
	if got, _ = mem.GetWorkload(context.Background(), clusterID, goneID); got != nil {
		t.Fatalf("unavailable OCI leftover must be removed: %+v", got)
	}
}
