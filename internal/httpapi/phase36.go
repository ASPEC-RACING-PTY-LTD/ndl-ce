...
// remapBusyHostPorts drops a published host port when it is already bound
// (the control plane listens on :8080). The container port stays mapped.
func remapBusyHostPorts(ports []oci.Port) []oci.Port {
	if len(ports) == 0 {
		return ports
	}
	out := append([]oci.Port{}, ports...)
	for i := range out {
		if out[i].HostPort <= 0 {
			continue
		}
		ln, err := net.Listen("tcp", fmt.Sprintf("127.0.0.1:%d", out[i].HostPort))
		if err != nil {
			out[i].HostPort = 0
			continue
		}
...
func (s *Server) installManifest(ctx context.Context, p *principal, m appmanifest.Manifest, name, poolID, networkID, nodeID, gpuID string, cpus int, memory int64) (stackID, workloadID string, err error) {
	...
	md.Ports = remapBusyHostPorts(md.Ports)
