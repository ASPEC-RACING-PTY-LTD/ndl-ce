...
func (s *Server) enableFeature(w http.ResponseWriter, r *http.Request) {
	...
	if mod.Package != "" {
		...
		if res.Reason != "" && row.PackageStatus == appdb.FeatureUnavailable {
			...
			row.Reason = strings.TrimSpace(reason + " " + mod.DefaultReason)
		}
...
func (s *Server) featureJSON(r *http.Request, clusterID string, mod features.Module) map[string]any {
	...
	out := map[string]any{
		...
		"reason":          row.Reason,
		"software_only":   mod.SoftwareOnly,
	}
	if mod.Package != "" {
...
func (s *Server) featureWorkloadCount(r *http.Request, clusterID, id string) int {
	switch id {
	case features.IDOCI, features.IDCT, features.IDVM:
		wls, _ := s.Store.ListWorkloads(r.Context(), clusterID)
		want := map[string]string{
			features.IDOCI: "oci",
			features.IDCT:  "system-container",
			features.IDVM:  "vm",
		}[id]
		n := 0
		for _, w := range wls {
	...
	case features.IDDocker:
		...
		s.docker.mu.Unlock()
		return n
	case features.IDGameServers:
		if s.Store == nil {
			return 0
		}
		rows, err := s.Store.ListGameServers(r.Context(), clusterID)
		if err != nil {
			return 0
		}
		return len(rows)
...
func featureDisableRequiresConfirm(mod features.Module, enabled bool, workloadCount int) bool {
	...
	switch mod.ID {
	case features.IDK8s, features.IDDistStorage, features.IDAI, features.IDDocker, features.IDGameServers:
		return true
	default:
		return false
	}
