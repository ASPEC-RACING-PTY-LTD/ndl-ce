...
IP="$(cert_discover_vm_ipv4 "$NODEB_WL" || true)"
...
"${SSH[@]}" "debian@${IP}" 'sudo DEBIAN_FRONTEND=noninteractive apt-get update -qq && sudo DEBIAN_FRONTEND=noninteractive apt-get install -y /tmp/ndl-agent_*.deb /tmp/nodalctl_*.deb'
