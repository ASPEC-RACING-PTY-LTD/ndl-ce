...
ensure_dirs() {
  ...
  # Cluster CA private key is written later by ndl-control (Ensure). The
  # process is unprivileged and cannot mkdir under the root-owned data dir.
  mkdir -p /var/lib/ndl/secrets
  chmod 0751 /var/lib/ndl/secrets
  mkdir -p /var/lib/ndl/secrets/cluster-ca
  chown ndl-control:ndl-control /var/lib/ndl/secrets/cluster-ca
  chmod 0700 /var/lib/ndl/secrets/cluster-ca
}
