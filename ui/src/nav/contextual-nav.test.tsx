...
describe("contextual navigation", () => {
  ...
  it("shows the live workload status from GET, not the stale catalog row", async () => {
    window.history.replaceState({}, "", "/workloads/wl-a");
    mockApi(admin, {
      "/api/v1/workloads/wl-a": { status: 200, body: { ...alpine, status: "stopped" } },
    });
    render(<App />);
    expect(await screen.findByRole("heading", { name: /^alpine$/i })).toBeVisible();
    expect(await screen.findByText(/system container · stopped/i)).toBeVisible();
    expect(document.querySelector(".ctx-item.is-current .ctx-dot.is-live")).toBeNull();
  });
