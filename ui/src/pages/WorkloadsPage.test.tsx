...
describe("Workloads bulk delete", () => {
  ...
  it("does not claim the cluster is empty while the list is still loading", async () => {
    mockApi(admin, {
      "/api/v1/workloads": () => ({ status: 200, body: { items: [], image_pins: [] } }),
    });
    ...
    window.history.replaceState({}, "", "/workloads");
    render(<App />);
    expect(await screen.findByText("Loading workloads")).toBeVisible();
    expect(screen.queryByText("No workloads yet")).not.toBeInTheDocument();
  });

  it("does not claim the cluster is empty when search matches nothing", async () => {
    await openWorkloads();
    fireEvent.change(screen.getByLabelText("Search workloads"), { target: { value: "zzzz-no-such" } });
    expect(await screen.findByText("No matching workloads")).toBeVisible();
