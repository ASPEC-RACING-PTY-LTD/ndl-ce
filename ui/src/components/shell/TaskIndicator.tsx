...
export function isRecentFailure(task: { state?: string; updated_at?: string }, now = Date.now()): boolean {
  if (!isFailed(task.state) || !task.updated_at) {
    return false;
  }
...
export function TaskIndicator() {
  ...
  const failed = items.find((t) => isRecentFailure(t));
  const label =
